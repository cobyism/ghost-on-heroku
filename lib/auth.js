...
const isAuthenticated = (req, res, next) => {
  ...
  // ignore certain paths from authentication
  if (/^\/(auth|ghost|assets|public|welcome)/.test(req.path)) {
    return next()
  }

  // if user logged in, pass through
  if (req.user) {
    return next()
  }
  ...
  res.redirect(utils.getSubdir() + '/welcome')
}
